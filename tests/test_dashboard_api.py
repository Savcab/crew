...
class SnapshotShape(unittest.TestCase):
    ...
    def test_agent_fields_present(self):
        ...
        self.assertTrue(a["cron_loops"] is None
                        or isinstance(a["cron_loops"], int),
                        f"bad cron count: {a['cron_loops']!r}")
        self.assertIn(
            a["live_status"],
...
# --------------------------------------------------------------------------- #
class WebhookAdmission(unittest.TestCase):
    def test_unknown_capability_is_rejected_before_declared_body_arrives(self):
        capability = "u" * 43
        request = (
            f"POST /hooks/{capability} HTTP/1.1\r\n"
        ...
        with socket.create_connection(
                ("127.0.0.1", int(PORT)), timeout=2.0) as client:
            client.settimeout(2.0)
            client.sendall(request)
            response = client.recv(4096)

        self.assertIn(b"HTTP/1.0 404", response)
        self.assertNotIn(capability.encode("ascii"), response)
